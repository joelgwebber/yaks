...
- **Linear:** issue + comments are separate calls (`get_issue` and `list_comments`). Use the normalized `statusType` (not display name) when mapping status: `backlog` and `unstarted` → hairy, `started` → shaving, `completed` → shorn, `canceled` → dead. Priority maps Linear 0–4 → yak 1–5: `1` (Urgent) → 1, `2` (High) → 2, `3` (Normal) → 3, `4` (Low) → 4, `0` (None) → 3 (yak default — "no upstream priority set" is treated as medium). Auto-resolve upstream-wins per the rubric. Attachments are first-class — `create_attachment` / `get_attachment` / `delete_attachment` actually work, so on this tracker `approve` on an `attachments_*` row is safe.

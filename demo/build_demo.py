...
exercises the rendering tools (tree list with ghosts, detail pane, state
moves). The full "herd evolving over time" story lands in a follow-up.
"""
...
from yakscreen import HAIRY, SHAVING, SHORN, Board, Layout
...
def screenplay() -> Cast:
    ...
    layout = Layout(COLS, ROWS, title="yaks demo")
    ...
    d.add("yak-4e08", "dark-mode polish pass", type="task", priority=4,
          labels=("ui",))
    d.tab(HAIRY)
    d._frame()
    d.beat(1.0)
    ...
    # A parent with children, plus a dependency, to exercise the tree + detail.
    ...
    d.detail("yak-1a2b", cursor=None)
    d._frame()
    d.beat(2.0)

    d.mark("Unblock")
    d.tool("$ yaks shorn yak-1a2b.1")
    ...
    d.mark("Shorn")
    d.detail(None)
    d.tool("$ yaks shorn yak-1a2b.2 && yaks shorn yak-1a2b")

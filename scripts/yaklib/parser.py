...
def _add_filter_flags(sp, exclude: tuple = ()) -> None:
    ...
    if "status" not in exclude:
        sp.add_argument("--status", choices=ALL_STATUS_NAMES, action="append", help="Filter by status (repeatable)")
    if "type" not in exclude:
        sp.add_argument("--type", action="append", help="Filter by type (repeatable)")
    if "priority" not in exclude:
        sp.add_argument("--priority", type=int, action="append", help="Filter by priority (repeatable)")
    if "label" not in exclude:
        sp.add_argument("--label", action="append", help="Filter by label — OR within labels (repeatable)")
    if "search" not in exclude:
        sp.add_argument("--search", help="Substring match on title/description/id")
    if "ready" not in exclude:
        sp.add_argument("--ready", action="store_true", help="Only tasks whose deps are all resolved")
    if "tangled" not in exclude:
        sp.add_argument("--tangled", action="store_true", help="Only tasks with at least one unresolved dep")
    if "parent" not in exclude:
        sp.add_argument("--parent-of", dest="parent_of", help="Only descendants of the given task ID")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp = sub.add_parser("slaughter", help="Slaughter a yak (move to hidden 'dead' state)")
    sp.add_argument("id", help="Task ID")
    ...
    for name in ("next", "ready"):
        sp = sub.add_parser(name, help="Show yaks ready to shave (shortcut for `list --status hairy --ready`)")
        _add_filter_flags(sp, exclude=("ready", "tangled", "status"))
        sp.add_argument("--json", action="store_true", help="JSON output")

    for name in ("tangled", "blocked"):
        sp = sub.add_parser(name, help="Show tangled yaks (shortcut for `list --status hairy --tangled`)")
        _add_filter_flags(sp, exclude=("ready", "tangled", "status"))
        sp.add_argument("--json", action="store_true", help="JSON output")
    ...
    sp = sub.add_parser("search", help="Search tasks by keyword (shortcut for `list --search QUERY`)")
    ...
    sp = sub.add_parser(
        "rollup", help="Group yaks by the external issue they point at (source: inherited from the nearest ancestor)"
    )
    _add_filter_flags(sp)
    sp.add_argument(
        "--keys", action="store_true", help="Print only the distinct external keys (for pasting into a PR body)"
    )
    sp.add_argument("--json", action="store_true", help="JSON output")
